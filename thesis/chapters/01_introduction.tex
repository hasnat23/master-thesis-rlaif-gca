...
\begin{enumerate}
  ...
  \item Evidence that the rule used to aggregate sentence-level scores is not a
    neutral implementation detail: a penalty-weighted aggregation performs
    measurably worse than a simple mean of the same scores
  ...
  \item A repeated-run, multi-scale evaluation establishing a statistically
    ...
    Wilcoxon $p<0.001$), and confirming, through twenty-run campaigns at
    $n=5{,}000$ and $n=10{,}000$ at the same resolution, that this
    advantage is genuinely absent rather than merely unreproduced at larger
    scale: both confidence intervals are centred on zero and neither overlaps
    the $n=1{,}000$ result (Sections~\ref{sec:results-extended-campaign}
  ...
  \item An independent, held-out ground-truth evaluation (RQ4) showing that the
    ...
    and~\ref{sec:results-gt-shortcut}). The held-out \emph{articles} are
    disjoint from every training pool, but the ranking both conditions are
    scored against is built from GCA's own aggregation rule, which qualifies
    the comparison (Chapter~\ref{ch:methodology},
...
Chapter~\ref{ch:results} reports the results, organised by research question:

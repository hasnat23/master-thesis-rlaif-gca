...
(Chapter~\ref{ch:results}, Section~\ref{sec:results-formula-opt}), and the
simple mean is the rule this thesis locks in and reports its confirmed result
with. That matters for the motivating example above: a plain mean of sentence
scores dilutes a single fabricated sentence in exactly the way a single
holistic score does, so GCA under its final, locked configuration does not
actually solve the fabricated-sentence case used to motivate it. What GCA is
shown to do well in this thesis is something narrower, produce a more
learnable training signal from independently scored sentences, not detect a
single fabricated sentence that a holistic judgment would miss. This
limitation is stated in full in Chapter~\ref{ch:conclusion},
Section~\ref{sec:concl-limitations}, and what survives of the granularity
question is narrower and more conditional than the motivating intuition
suggests.

...
\begin{enumerate}
  \item A controlled experimental framework for comparing holistic and
    sentence-level (GCA) construction of factuality preferences for
    abstractive summarization, in which the candidate pool, evaluator,
    reward-model architecture, and training procedure are held fixed so that
    feedback granularity is isolated as the only manipulated variable
    (Chapter~\ref{ch:methodology}).

  \item Granular Credit Assignment (GCA) itself: a parameterized rule for
    aggregating sentence-level factuality scores into a single preference
    label (Chapter~\ref{ch:methodology}, Section~\ref{sec:bg-gca}), together
    with a search protocol for selecting among alternative aggregation
    strategies empirically rather than fixing one by assumption
    (Chapter~\ref{ch:results}, Section~\ref{sec:results-formula-opt}).

  \item A repeated-run, multi-scale evaluation protocol, combining twenty
    independently seeded training runs at each of three dataset sizes with
    equivalence testing, that distinguishes a genuine absence of an effect
    from a merely undetected one (Chapter~\ref{ch:results},
    Sections~\ref{sec:results-extended-campaign}
    and~\ref{sec:results-effect-sizes}).

  \item An independent, held-out ground-truth evaluation protocol that
    scores trained reward models against data disjoint from every training
    pool used in this thesis, designed to test whether a learnability
    advantage measured under cross-validation also holds against
    out-of-sample data (Chapter~\ref{ch:methodology},
    Section~\ref{sec:methodology-ground-truth}).

  \item A fully seeded, reproducible experimental pipeline, released with
    this thesis, with documented data provenance, including a discrepancy
    between the intended and the actually-used evaluator configuration that
    was found and disclosed during final review
    (Chapters~\ref{ch:implementation} and~\ref{ch:conclusion}).
\end{enumerate}

Applying this framework produces the empirical results reported in
Chapter~\ref{ch:results}: a statistically significant GCA advantage at
$n=1{,}000$ that is absent at $n=5{,}000$ and $n=10{,}000$, and a
ground-truth precision advantage at $n=1{,}000$ that reverses, rather than
merely vanishes, by $n=10{,}000$. These results concern the construction and
learnability of preference data. The thesis does not claim that GCA yields
objectively more accurate factuality labels, nor that it improves the
factual consistency of generated summaries; the scope of what the
experiments support is stated precisely in Chapter~\ref{ch:methodology},

...
Section~\ref{sec:methodology-evolution}. The evaluation mode and $\alpha$
below are the \emph{intended} configuration; Chapter~\ref{ch:implementation},
Section~\ref{sec:impl-preferences} reports that the twenty-run campaign
actually reproduced by the commands in
Section~\ref{app:repro-commands} was trained on preference data built
under \texttt{nli\_sp} and $\alpha=0.5$, not the \texttt{nli} and
$\alpha=0.0$ shown here, an implementation oversight found during final
review. Running the commands below as written reproduces the
\emph{intended} configuration, which is \texttt{nli}/$\alpha=0.0$, not the
one the headline campaign was actually trained on.
...
\noindent
Step 3 above, run exactly as written, produces the intended
\texttt{nli}/$\alpha=0.0$ preference data. The actual headline campaign
was produced by the Slurm scripts \url{slurm/build_preferences_1000.sh}
and \url{slurm/build_preferences_scale.sh} (below) instead of this exact
command, and those scripts omit \texttt{--alignscore-evaluation-mode}
(defaulting to \texttt{nli\_sp}) and, for $n=1{,}000$, pass
\texttt{--alpha 0.5}; this is the discrepancy reported in
Chapter~\ref{ch:implementation}, Section~\ref{sec:impl-preferences}. A
reader following the four commands above exactly as shown will reproduce
the configuration this thesis intended, not the one the headline numbers
in Chapter~\ref{ch:results} were actually measured from.
...
summary's weakest sentence, the behavior that Chapter~\ref{ch:results},

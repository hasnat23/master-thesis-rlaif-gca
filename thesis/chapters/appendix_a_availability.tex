...
Chapter~\ref{ch:implementation} describes in prose. The reproducibility caveat
stated in Chapter~\ref{ch:implementation}, Section~\ref{sec:impl-repro} applies:
because PyTorch's generator is not seeded in the cross-validation path, two
executions with identical arguments can differ, and these commands reproduce the
experimental \emph{setup} rather than guaranteeing bit-identical numbers.

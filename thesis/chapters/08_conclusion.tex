...
advantage does not remain equally large as the dataset grows: it is approximately
$-0.42$ percentage points at $n=5{,}000$ and $+0.35$ percentage points at
$n=10{,}000$ (each a single run), while absolute accuracy improves for both
conditions. Uncertainty estimates computed over cross-validation folds remain
anti-conservative, because folds within a run are not independent, but the
run-level estimate at $n=1{,}000$ is now supported by a properly powered,
significant test.
...
scoring?) is answered \emph{conditionally, but with statistical support at
$n=1{,}000$}. An average advantage of $+3.95$ percentage points is observed
across twenty independent runs at $n=1{,}000$, favouring GCA in all twenty, with
a run-level Wilcoxon $p<0.001$ and a bootstrap confidence interval that does not
cross zero. It is not observed in the single run at $n=5{,}000$ and is marginal
at $n=10{,}000$. The answer is therefore statistically supported at the dataset
size where it was most thoroughly tested, but configuration- and
scale-dependent rather than general.
...
answered \emph{differently across the two axes it asks about}. Across repeated
runs at a fixed dataset size, the difference is highly stable: all twenty runs
of the extended campaign at $n=1{,}000$ favour GCA, with a standard deviation of
1.78 percentage points around a mean of $+3.95$. Across dataset sizes, stability
does not hold: the advantage is not reproduced with comparable magnitude in the
single runs at $n=5{,}000$ and $n=10{,}000$, which use nested supersets of the
same data (Section~\ref{sec:disc-scale}). Run-to-run stability at fixed scale is
therefore well established; stability across scale is not, and only one run was
performed at each of the two larger sizes.

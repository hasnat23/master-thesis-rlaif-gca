...
\citet{casper2023open} catalogue failure modes of this kind across the RLHF
...
Two more recent lines of work place sentence-level signals at other points in the
alignment pipeline, and both should be distinguished carefully from the present
study, since neither leaves the question examined here open in general.
...
\citet{qiu2026sentencelevel} modify the reward model itself, proposing a
...
\citet{qiu2026sentencelevel} & the reward-model architecture &
...
\citep{qiu2026sentencelevel}, to supply dense rewards during reinforcement
learning \citep{wu2023finegrained}, and to build evaluation metrics
\citep{laban2022summac, song2024finesure}. What distinguishes this thesis is the
...
\begin{itemize}
  ...
  \item \textbf{Sentence-level reward modeling} \citep{qiu2026sentencelevel}
  ...
  \item \textbf{Fine-grained evaluation frameworks}
    \citep{laban2022summac, song2024finesure} decompose summaries in order to
    diagnose them rather than to supervise with them.
\end{itemize}

The comparison is conducted on preference data generated by one evaluator on one
corpus, so the findings describe behaviour under specific conditions rather than a

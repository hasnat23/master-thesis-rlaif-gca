...
ordinary run-to-run variation, so each scale was subsequently extended to six
independently seeded runs under the same corrected, deterministic training
procedure used for the $n=1{,}000$ extension
(Section~\ref{sec:results-extended-campaign}).

\begin{table}[htbp]
\centering
\caption[Repeated-run statistics at all three dataset sizes]{Repeated-run statistics at all three dataset sizes. The $n=1{,}000$ row
is the twenty-run campaign from Table~\ref{tab:seed-campaign-stats}; the
$n=5{,}000$ and $n=10{,}000$ rows are six-run campaigns using the same
...
5{,}000  & 6  & 2/6   & $+0.02$pp & $[-0.44, +0.62]$pp & $0.6875$ \\
10{,}000 & 6  & 4/6   & $-0.03$pp & $[-0.54, +0.42]$pp & $1.0000$ \\
\bottomrule
\end{tabular}
\end{table}

At $n=5{,}000$ and $n=10{,}000$ the mean gap is close to zero in both cases,
GCA leads in roughly half the runs at each scale rather than consistently, and
the two-sided Wilcoxon tests give $p=0.6875$ and $p=1.0000$ --- results
indistinguishable from chance. This is not merely a failure to reach
significance in the way the original six-run $n=1{,}000$ campaign was: with six
runs split close to evenly between the two directions, these are not
underpowered positive results but genuinely null ones.
...
$n=1{,}000$ interval, $[+3.19, +4.70]$pp, does not overlap $[-0.44,
+0.62]$pp at $n=5{,}000$ or $[-0.54, +0.42]$pp at $n=10{,}000$, and the gap
...
evidence at each scale rather than by three single runs of unequal weight.

\begin{table}[htbp]
\centering
\caption[Selected configuration at three dataset sizes, repeated-run means]{Selected configuration at three dataset sizes. Means at $n=5{,}000$ and
$n=10{,}000$ are now averaged over six runs each, not single runs.}
\label{tab:scale-robustness}
\begin{tabular}{rrrr}
\toprule
Dataset size & Holistic mean & GCA mean & Gap (GCA $-$ Holistic) \\
\midrule
1{,}000  & 0.5295 & 0.5603 & $+0.0395$ \\
5{,}000  & 0.5855 & 0.5857 & $+0.0002$ \\
10{,}000 & 0.5857 & 0.5854 & $-0.0003$ \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[holbar] coordinates {(1{,}000,0.5295) (5{,}000,0.5855) (10{,}000,0.5857)};
\addplot[gcabar] coordinates {(1{,}000,0.5603) (5{,}000,0.5857) (10{,}000,0.5854)};
...
\end{axis}
\end{tikzpicture}
\caption[Accuracy at the three dataset sizes]{Accuracy at the three dataset sizes, all now averaged over repeated
runs (20 at $n=1{,}000$, 6 each at $n=5{,}000$ and $n=10{,}000$). Absolute
accuracy rises for both conditions as data is added, while the two
conditions converge to statistically indistinguishable means at the larger
scales.}
\label{fig:scale-results}
\end{figure}

Absolute accuracy continues to rise with dataset size for both conditions ---
from roughly 0.53--0.56 at $n=1{,}000$ to roughly 0.585--0.586 at the larger
...
RQ1 &
  ...
  $p=0.0625$). Six-run campaigns at $n=5{,}000$ and $n=10{,}000$
  (Section~\ref{sec:results-scale}) confirm the advantage is genuinely absent
  at those scales, not merely unreproduced: mean gaps of $+0.02$pp
  ($p=0.6875$) and $-0.03$pp ($p=1.0000$), with confidence intervals that do
  not overlap the $n=1{,}000$ result. \\
RQ2 &
...
RQ3 &
  ...
  favour GCA (SD 1.78pp); six-run campaigns at $n=5{,}000$ and $n=10{,}000$
  are similarly stable around a mean of approximately zero. The advantage

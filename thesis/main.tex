...
% Embed document metadata so the PDF identifies itself in readers and library
% catalogues rather than showing the filename.
\hypersetup{
  pdftitle={Benchmarking Holistic vs Sentence-Level Reward-Model Preferences
            for Factual Summarization via Granular Credit Assignment (GCA)},
  pdfauthor={Muhammad Hasnat},
  pdfsubject={Master's Thesis, Web and Data Science, University of Koblenz},
  pdfkeywords={RLAIF, reward modeling, factual consistency, summarization,
               preference learning, Granular Credit Assignment},
}
...
+4.70]$, run-level Wilcoxon $p<0.001$, Cohen's $d=2.22$).

\textbf{RQ2} asks how evaluator configuration and aggregation strategy affect this
comparison. The aggregation rule matters substantially: an early weakest-link
penalty formula measurably hurt GCA relative to a simple mean. A single-run
sweep across four AlignScore modes suggested the evaluator mode matters too,
but the confirmatory campaigns do not bear that out, so it is reported as
exploratory rather than established.

\textbf{RQ3} asks whether the RQ1 advantage is stable across dataset scale; it is
not. Twenty-run campaigns at $n=5{,}000$ and $n=10{,}000$, at the same
resolution, give mean gaps of $-0.22$ and $-0.11$ percentage points, both
statistically indistinguishable from zero, with equivalence tests excluding
any effect larger than $\pm0.61$ and $\pm0.36$ points: the advantage is
absent, not merely undetected. Absolute accuracy improves for both conditions
as data grows even as their difference vanishes.
...
\input{chapters/appendix_d_exploratory}
